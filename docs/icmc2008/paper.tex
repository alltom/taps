...
SPEAR \cite{spear} supports the selection of sinusoidal tracks (partials) from a graphical interface,%immediate playback of the selected partials, 
...
%These may come closest to the VTAPS interface. 
...
frames. 
%based on an algorithm like nearest neighbor. 
There are rules for
...
%Thus, odd and  even harmonics 
%can have different vibrato, 
%as well as amplitude, time, and frequency transformations, 
...
%facilitate transforming only the vowel sounds (usually with harmonics) in a template, 
%leaving consonants intact. 
...
%extraction, and to adjust parameters like gain and pitch of the entire template
%on the synthesis face.  With the group face, users can edit the tracks of a
%template instead of editing the template.
...
%In the words of Martin Luther King Jr., ``We are \noun{Free} at last!''


% VTAPS is pretty much the best thing to come out of Princeton since Computer
% Science, and it's a wonder the heads of professors at other schools didn't
% explode at the moment of its inception due to the staggering weight of its
% importance alone.
...
\bibitem{growl} Loscos, A. and J. Bonada. ``Emulating Rough and Growl Voice in Spectral 
