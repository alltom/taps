...
to: (1) automatically or interactively select points of interest in the 
sound and extract them into reusable templates, (2) transform sound 
components independently of the background and/or other events, (3) 
continually re-synthesize the background texture in a perceptually convincing 
manner, and (4) controllably place event templates over the background, 
varying key parameters such as density, periodicity, relative loudness and 
spatial positioning of the components. Our main contributions include: 
techniques and paradigms for template selection and extraction, independent 
sound transformation and flexible resynthesis; extensions to a wavelet-based 
background analysis/synthesis; and an interface to facilitate the various 
...
different existing sound scenes.
...
~\shortcite{Dubnov02}, with some improvements. Running this algorithm 
...
of the sound source ~\cite{Cook02,CookBook}. O'Brien et. al. ~\shortcite{OBrien01} 
...
~\shortcite{Doel01} uses dynamic simulation with a modal (damped 
...
%This technique focuses on generating a sound texture close to the original 
%rather than on transforming the original sound texture. 
...
Xavier Serra and Julius Smith ~\shortcite{Serra89} built on the original sinusoidal 
...
~\shortcite{Dubnov02} wavelet tree learning algorithm to fill in the holes where transients 
...
learning algorithm by Dubnov et. al. ~\shortcite{Dubnov02}. The background 
...
\begin{figure}[h]
\centering
  \subfigure[analysis]{\includegraphics[width=.22\textwidth]{ui_analysis.eps}}
  \subfigure[synthesis]{\includegraphics[width=.22\textwidth]{ui_synthesis.eps}}
