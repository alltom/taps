...
void envExtr(int bufLen, float *bufin, float *env, float attack = .8,
	         float decay = .9);
int transExtr(int envLen, float *env, int *transientIndices, float thresh,
	          float ageamt = .9, int allowableGap = 1000);
...
t_CKUINT UIAnalysis::render( void * data )
{
    ...
    // flippers
    draw_flipper( ui_elements[BT_GET_TRANSIENT], .8f, -0.05f, 0.0f );
                    ...
                    if( ie->state == ae_input_UP && ui_elements[i].down == TRUE )
                    {
                        ...
                        else if( i == BT_SEPARATE )
                        {
                            ...
                            else
                            {
                                ...
                                // transient
                                if( ui_elements[BT_GET_TRANSIENT].slide > .5f )
                                {
                                    if( !get_transient( m_filename.c_str(), start, end ) )
                                    {
                                        msg_box( "ah", "need more effort on transients" );
                                    }
                                }
                            }
                        }
