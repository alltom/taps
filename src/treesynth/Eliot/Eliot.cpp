...
double Treesynth::FindEpsilon( Tree * thetree, int size, double P )
{
    ...
    return 2.0 * (double)ourtree[index];
}
...
void Treesynth::CandidateSet( int level, int offset ) {
	...
	if(offset % 2 == 1)
    {
        if( tnew_data->getNode( level, offset )->parent->cs ) {
            delete [] tnew_data->getNode( level, offset )->parent->cs;
            tnew_data->getNode( level, offset )->parent->cs = NULL;
            tnew_data->getNode( level, offset )->parent->cslength = 0;
        }
    }
	if(level == tree->getLevels() - 2 && offset > 0)
    {
        if( tnew_data->getNode( level, offset-1 )->cs ) {
            delete [] tnew_data->getNode( level, offset - 1 )->cs;
            tnew_data->getNode( level, offset - 1 )->cs = NULL;
            tnew_data->getNode( level, offset - 1 )->cslength = 0;
        }
    }
...
TreesynthIO::TreesynthIO()
{
	...
	nsamples = TS_BIG_BUFFER_SIZE;

	leftinit = false;

	rm_next_pos = 0;
	rm_next_length = CUTOFF;
...
// desc: audio callback
//-----------------------------------------------------------------------------
int TreesynthIO::m_audio_cb( char * buffer, int buffer_size, void * user_data )
{
    ...
    else
    {
        ...
        if( !g_audio_begin )
        {
            ...
            g_audio_end = g_audio_begin + nsamples;
            g_read_index %= g_max_data_count;
        }
        
        // copy the data out
        memcpy( buffer, g_audio_begin, buffer_size * sizeof(TS_FLOAT) );
