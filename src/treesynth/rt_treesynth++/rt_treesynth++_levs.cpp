/*
	...
	Toggles:
		randflip - randflip on or off (false in original)
		ancfirst - ancestor first or not (true in original)
		use_left_tree - whether to use a left tree or not (false in original)
		use_kfactor - whether to use kfactors or a constant (false in original)
		g_ready - 1 or 0, whether to fill up buffers before anything is played (if 0)
		write_to_file - whether to write to file or play in rtaudio
	Can also change start and stop levels : startlevel, stoplevel (=0 means it will stop at highest possible level)
*/ 
...
// treesynth knobs
bool use_kfactor = true;
int npredecessors = 5; // number of predecessors checked (dummy value of 3) [5 in dubnov's examples]
...
double percentage = .15; // percentage of nodes to be considered when learning new tree [60 to 70 in dubnov's egs]
...
int startlevel = 1;
int stoplevel = 9; //9; //0; // changed later in the program
enum TheReadModes
{
    RM_STOP = 0x0,
    RM_WRAP = 0x1,
...
int rm_mode = RM_FORWARD | RM_WRAP; // what direction and what to do at boundary
...
int g_ready = 1; // for filling up buffers before anything is played (nonintuitive)
	...
	for(int q = 0; q < csl; q++) {
		...
		if(in[q]){
			...
			for(int z = 1; z <= npred; z++) { // npred instead of np
				...
				else if( lengths[lev] + cands[q] - z >= 0 && lefttree && use_left_tree )
...
void treesynth( void )
{
	...
	for(lev = startlevel; lev <= stoplevel; lev++) {				
		...
		npredecessors = use_kfactor ? (int)(lengths[lev] * kfactor) : npredecessors;  
		cout << "k is " << npredecessors << endl;
...
double end_time = clock();
	...
	cerr << "\n\n\nTotal processing time: " << (end_time - begin_time)/CLOCKS_PER_SEC << "seconds" << endl;; 

	if( sfwrite ) sf_close( sfwrite );
