...
public:
    ...
    void setCode (t_TAPUINT id ) { _code = id; } 
    t_TAPUINT getCode (); 
    void setMenu ( UIMenu * menu);
    void fitLabel ( t_TAPBOOL mono = false );
    void fitXLabel ( double tscale, t_TAPBOOL mono = false );
    t_TAPUINT buttonID() { return _code ; }
    //fnptr _callback();
};
...
struct menuItem { 
    ...
    UIButton    button;
    menuItem();
    menuItem(char* c, t_TAPUINT code );
    void reshape();
};
...
protected :
    ...
    double                      _margin;
    UIButton                    _button;
    t_TAPUINT                    _default_code;
public:
    UIMenu();
    void draw();
    void reshape();
    void addOption(char *c, t_TAPUINT code = 0);
    virtual void handleMouse( const InputEvent &e);
    t_TAPUINT getCode( );
    void addOption(std::string s, t_TAPUINT code = 0) { addOption( s.c_str(), code ); }
