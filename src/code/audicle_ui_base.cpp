...
UIRectangle::roundVerts( float rad) {
    ...
    int div = 3;
    double span = TWOPIE * 0.25 / ( double)div;
    int i;

    if ( rad <= 0 ) rad = 0.5 * x_min ( w( ), h( ) );
...
UIRectangle::bubbleRounded( float rad ) { 
    ...
    roundVerts( rad);
    double r = ( rad > 0 ) ? rad : 0.5 * x_min ( h( ), w( ));
...
UIButton::drawLabel( float scale , int mono) { 
    glPushMatrix();
        ...
        if ( mono ) drawString_centered_mono ( _label.c_str() );
        else drawString_centered(_label.c_str() );
    glPopMatrix();
}
...
void
UIButton::fitLabel( t_TAPUINT mono ) { 
    double dlen = 1.0;
    if ( mono ) dlen = 0.01 * drawString_length_mono( _label.c_str() );
    else dlen = 0.01 * drawString_length( _label.c_str() );
    setw( h() * x_max ( 1.0 , dlen )) ;
}

void
UIButton::fitXLabel( double tscale, t_TAPUINT mono ) { 
    fitLabel(mono);
    setw( w() * tscale * 0.75 + 0.75 * h() );
}
...
menuItem::menuItem ( char * n, t_TAPUINT c ) { 
    name = n;
    code = c;
    reshape();
}
...
{ 
    ...
    outlineRounded(0.5);
    glPushMatrix();
    for ( t_TAPUINT i = 0 ; i < _options.size() ; i++ ) { 
