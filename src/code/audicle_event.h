...
struct InputEvent {
    ...
    void setMods( t_TAPUINT imods )                      { mods  = imods; } 
    void setStack( t_TAPUINT * istack, t_TAPUINT isize )  { stack = istack; size = isize; }
    Point2D p() { return pos; } 
    
    void popStack( ); 
    void fprint( FILE * fd) const; 
    t_TAPUINT curID( void ) const;
    bool checkID( t_TAPUINT id ) const;
};
