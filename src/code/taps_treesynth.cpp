...
int lg( int n ) { 
    ...
    if( n <= 0 ) {
        BB_log( BB_LOG_WARNING, "Treesynth, lg : Sorry, I'm a hack and only compute approximate logs of positive integers" );
        exit(2);
    }
    int log = 0;
    int result = 1;
...
{
    if( levels < 1 ) {
        BB_log( BB_LOG_WARNING, "Tree::initialize : levels (%i) must be > 1", levels );
		return false;
    }

    if( levels > 20 ) {
		BB_log( BB_LOG_WARNING, "Tree::initialize : levels (%i) is too big for me. Retrench!", levels );
        return false;
    }
...
{
    if( lefttree ) {
        BB_log( BB_LOG_WARNING, "Treesynth: Sorry, cannot reset levels because \
				lefttree will get messed up" );
        return;
    }
...
TreesynthIO::TreesynthIO()
{
    ...
    BB_log( BB_LOG_INFO, "TreesynthIO: You have created a TreesynthIO object." );
...
//        sf_close( sfwrite );
    ...
    BB_log( BB_LOG_FINE, "TreesynthIO : Goodbye!" );
}
...
int TreesynthIO::m_audio_cb( char * buf, int buffer_size, void * user_data )
{
    ...
    while( remaining > 0 )
    {
        ...
        int tocopy = our_min<long>( remaining, g_audio_end - g_audio_begin );
...
//        sfread = sf_open( filename, SFM_READ, &readinfo );
        ...
        if( !sfread )
        {
            BB_log( BB_LOG_WARNING, "TreesynthIO::ReadSoundFile : cannot open file '%s', quitting", filename );
            return 0;
        }
    }
	...
	if(itemsread < datasize && itemsread < readinfo.frames) {
		BB_log(BB_LOG_FINE, "Requested: %d, got: %d, available: %d; trying again", 
	 					 datasize, itemsread, readinfo.frames);
	...
	BB_log(BB_LOG_FINE, "Returning: %d\n", itemsread);
    return itemsread;
}
...
//            sfwrite = sf_open( filename, SFM_WRITE, &writeinfo );
			sfwrite = AudioCentral::instance()->m_cachemanager->opensf( filename, SFM_WRITE, &writeinfo );

            if( !sfwrite )
            {
                BB_log( BB_LOG_WARNING, "TreesynthIO::WriteSoundFile : cannot open file '%s', quitting", filename );
                //char x[256];
                //std::cin.getline( x, 256 );
                //exit(1);
                return 2;
        ...
        if( itemswritten <= 0 ) {
			BB_log( BB_LOG_WARNING, "TreesynthIO::WriteSoundFile : cannot write to file '%s', quitting", filename );
            //char x[256];
            //std::cin.getline( x, 256 );
            //exit(3);
            return 2;
