...
void Syn::synthesize( Frame & frame )
{
	...
	TIME start_time = tracks[0]->history[0].time;		// danger comment: timeout
	TIME end_time = tracks[0]->history.back().time;	// danger comment: timeout
	...
	// round to uint because that worked in old version
	start_time = (uint)(start_time * time_stretch);
	end_time = (uint)(end_time * time_stretch);
	...
	for( t = 0; t < tracks.size(); t++ ) {
        ...
        halt.time += 256 / time_stretch; //BirdBrain::hop_size(); // 256
		...
		for( int offset = 1; offset < tracks[t]->history.size(); offset++ ) {
			...
			// add f back to frame
			frame.add( f, 0, (uint)(time_stretch * tracks[t]->history[offset-1].time) - start_time );
		}
...
void Syn::synhelp( Frame &frame, Track *t, int offset )
{
    ...
    diff_t = (uint)(time_stretch*curr.time) - (uint)(time_stretch*prev.time);
...
void SynFast::synthesize( Frame &frame )
{
	...
	TIME start_time = tracks_info[0].start_time;
	TIME end_time = tracks_info[0].end_time;
	...
	for( int i = 0; i < tracks.size(); i++ )
	{
		...
		if( time_stretch * trin.start_time <= time_stretch * cur_time + frame.wsize 
			&& time_stretch * (trin.end_time + 128) > time_stretch * cur_time )
		{
...
void SynFast::synhelp( Frame &frame, int trackind )
{
	...
	while( j < tocompute )
	{
		...
		if( ti->histind < t->history.size() - 1 )
			...
			next = prev;
			next.p.mag = 0.f;
			next.time += 128; //BirdBrain::hop_size(); // 256
		}
		...
		BB_log( BB_LOG_FINEST, "j: %i, tocompute: %i, diff_t: %f, ti->diff_t: %f, ti->ratio_t: %f", j, tocompute, diff_t, ti->diff_t, ti->ratio_t ); 
		...
		while( j < tocompute && diff_t > 0 )
		{
			...
			// tracks with just one frame
			if( ::fabs(ti->ratio_t - 0.5f) < 0.0001 && ti->single_point_track )
			{
				delta_f = -delta_f;
				delta_m = -delta_m;
			}
