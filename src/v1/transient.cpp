...
bool EnvExtractor::extract( uint start, uint end )
{
    ...
    if( estart != start || eend != end || need_to_read )
    {
        ...
        // get samples
        file.mtick( read_frame->waveform, buffsize );
...
bool EnvExtractor::remove_transients( const char *outfilename, std::vector< uint > rmindexs )
{
...
bool EngExtractor::extract( uint start, uint end )
{
  int buffsize = end - start + 1; // + 1?
  estart = start;
  eend = end;
  ...
  if( numTransients )
  {
    for(int i = 0; i < numTransients; i++)
    {
        fprintf( stderr, "%i start %i end %i buffsize %i\n", i, transients[i].start, 
...
bool EngExtractor::remove_transients( const char * outfilename, std::vector<uint>rmindexs )
{
    return TransientExtractor::remove_transients( outfilename, rmindexs );
}
