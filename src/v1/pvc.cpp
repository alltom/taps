...
// desc: ...
//-----------------------------------------------------------------------------
PVC::PVC( uint window_size, uint io_size, uint pool_size )
{
...
// desc: ...
//-----------------------------------------------------------------------------
void PVC::pv_analyze( SAMPLE * buffer, uint hop_size )
{
...
// desc: ...
//-----------------------------------------------------------------------------
void PVC::pv_set_window( uint type )
{
    if( type == PV_HAMMING )
...
void PVC::pv_unwrap_phase_( const polar_window * prev, polar_window * curr )
{
    ...
    for( int i = 0; i < len; i++ )
    {
        omega = 2 * PIE * _hop_size * i / len; // hop size seemingly useless at this point
        delta_phi = princarg( cold[i].phase - ( pold ? pold[i].phase : 0 ) - omega );
        _phase_inc[i] = (omega + delta_phi); 
    }
}
...
//-------------------------------------------------------------------------------------------------
void PVC::signal_swap( SAMPLE * sig, uint length )
{
    SAMPLE temp;
...
void PVC::pv_phase_fix_( const polar_window * prev, polar_window * curr, float time_stretch )
{
    ...
    for( int i = 0; i < curr->len; i++ )
    {
        curr->array[i].phase = princarg(prev->array[i].phase + time_stretch * _phase_inc[i]); // take princarg in later iteration
        // princarg apparently matters: if we use it, pvc either 'hangs' (if time_stretch is actually syn hop size)
        // or goes back to changing freq during time stretch (if time_stretch is actually time_stretch as in paper)
    }   
    return;
}
...
// desc: ...
//-----------------------------------------------------------------------------
void PVC::pv_overlap_add( polar_window * the_window, uint hop_size )
{
//    assert( hop_size <= _window_size );
    if( hop_size > _window_size/2 ) hop_size = _window_size/2;

    uint to_ola = _window_size - hop_size;
    SAMPLE * window = _space2;
    SAMPLE * w = _ola[_index];
    _index = !_index;
...
// desc: ...
//-----------------------------------------------------------------------------
uint PVC::pv_synthesize( SAMPLE * buffer )
{
    if( _ready.size() == 0 )
        return FALSE;
...
uint PVC::pv_get_ready_len()
{
    return (uint)_ready.size();
}
...
// desc: constructs
//-----------------------------------------------------------------------------
PVCtoo::PVCtoo( SAMPLE * orig, uint length, uint ana_hop_size, uint window_size, uint io_size, uint pool_size ) 
...
PVC * PVCtoo::pv_copy()
{
    ...
    for( int i = 0; i < _windows.size(); i++ )
    {
        ...
        // copy
        polar_window * q = _windows.get(i); 
        p->len = q->len;
        memcpy( p->old, q->old, q->len * sizeof(polar) );
        memcpy( p->array, p->old, p->len * sizeof(polar) );
